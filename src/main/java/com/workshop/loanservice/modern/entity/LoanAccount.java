...
/**
 * Modern loan account (maps to the {@code loan_accounts} table). Replaces legacy
 * {@code CDW_LN_ACCT}: borrower data is no longer denormalized — it is referenced
 * through a {@link Borrower} foreign key — and the product is referenced through
 * a {@link LoanProduct} foreign key.
 */
@Entity
