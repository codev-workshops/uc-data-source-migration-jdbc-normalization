...
/**
 * Reads from the modern, normalized schema and maps it to the existing DTOs.
 *
 * All type parsing now lives in the data layer (proper LocalDate / BigDecimal /
 * Integer columns). What remains is purely presentation: the API contract still
 * exposes dates as MM/DD/YYYY strings, human-readable statuses, and composite
 * property address / borrower name. Those rules are preserved exactly so the
