...
/**
 * Builds the API DTOs straight from the legacy tables, exactly the way
 * {@code LoanService} did before the migration.
 *
 * <p>This is reconciliation-only code: it gives {@code ValidationController} a legacy
 * baseline to compare the modern-backed responses against, and it is the reference
 * implementation to fall back on if a rollback is ever needed.
 */
@Component
public class LegacyDtoAssembler {
    ...
    public LegacyDtoAssembler(LegacyBorrowerRepository borrowerRepository,
    ...
    public long countBorrowers() { return borrowerRepository.count(); }

    public long countLoanProducts() { return loanProductRepository.count(); }

    public long countLoanAccounts() { return loanAccountRepository.count(); }

    public long countPayments() { return paymentRepository.count(); }
