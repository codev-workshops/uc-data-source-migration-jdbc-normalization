...
-- MODERN SEED DATA
-- =============================================================================
-- The same five borrowers, five products, five loan accounts and ten payments
-- that the legacy CDW seed contained, transformed per data/mappings/column_mappings.md:
--   * amounts: commas stripped, stored as DECIMAL
--   * dates:   MM/DD/YYYY parsed into DATE / TIMESTAMP
--   * codes:   expanded (ACT -> ACTIVE, SFR -> SINGLE_FAMILY, REG -> REGULAR, PST -> POSTED)
--   * ids:     legacy identifiers retained as external_id / account_number
-- =============================================================================

-- Borrowers (external_id = legacy BORR_ID)
...
-- Loan Products (code = legacy PROD_CD)
...
-- Loan Accounts (account_number = legacy LN_ACCT_NBR; borrower/product resolved via FK lookups)
...
('LN-2017-00034', (SELECT id FROM borrowers WHERE external_id = 'B-10005'), (SELECT id FROM loan_products WHERE code = 'FHA30'), 165000.00, 142567.90, 4.250, 360, 811.61,  DATE '2017-03-01', DATE '2047-03-01', DATE '2017-04-01', DATE '2026-01-01', 'ACTIVE', 0,  1890.45, 80.00, '2200 Cedar Lane', 'Phoenix',     'AZ', '85001', 'SINGLE_FAMILY', 206000.00, TIMESTAMP '2017-02-20 00:00:00', TIMESTAMP '2025-12-01 00:00:00');

-- Payments (external_id = legacy PMT_SEQ_NBR; loan resolved via FK lookup)
