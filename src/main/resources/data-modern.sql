...
-- MODERN SEED DATA
-- =============================================================================
-- Migrated from the legacy CDW seed data (data-legacy.sql) using the mappings in
-- data/mappings/column_mappings.md: dates parsed from MM/DD/YYYY, amounts stripped
-- of commas, codes expanded to their modern values.
-- =============================================================================

INSERT INTO borrowers (external_id, first_name, last_name, middle_initial, ssn_hash, date_of_birth,
                       address_line1, address_line2, city, state, zip_code, phone, email,
                       credit_score, employment_status, annual_income, status, created_at, updated_at) VALUES
...
INSERT INTO loan_accounts (account_number, borrower_id, product_id, original_amount, current_balance, interest_rate,
                           term_months, monthly_payment, origination_date, maturity_date, first_payment_date,
                           next_payment_date, status, delinquency_days, escrow_balance, ltv_percent,
                           property_address, property_city, property_state, property_zip, property_type,
                           appraised_value, created_at, updated_at) VALUES
('LN-2019-00142', (SELECT id FROM borrowers WHERE external_id = 'B-10001'), (SELECT id FROM loan_products WHERE code = 'FXD30'), 285000.00, 271432.56, 4.750, 360, 1487.02, DATE '2019-02-15', DATE '2049-02-15', DATE '2019-03-15', DATE '2026-01-15', 'ACTIVE', 0, 3245.80, 82.50, '742 Elm Street', 'Springfield', 'IL', '62701', 'Single Family Residence', 345000.00, TIMESTAMP '2019-02-01 00:00:00', TIMESTAMP '2025-12-01 00:00:00'),
...
('LN-2017-00034', (SELECT id FROM borrowers WHERE external_id = 'B-10005'), (SELECT id FROM loan_products WHERE code = 'FHA30'), 165000.00, 142567.90, 4.250, 360, 811.61, DATE '2017-03-01', DATE '2047-03-01', DATE '2017-04-01', DATE '2026-01-01', 'ACTIVE', 0, 1890.45, 80.00, '2200 Cedar Lane', 'Phoenix', 'AZ', '85001', 'Single Family Residence', 206000.00, TIMESTAMP '2017-02-20 00:00:00', TIMESTAMP '2025-12-01 00:00:00');

INSERT INTO payments (external_id, loan_account_id, payment_date, total_amount, principal_amount, interest_amount,
                      escrow_amount, late_fee, type, status, received_date, processed_date, created_at, updated_at) VALUES
